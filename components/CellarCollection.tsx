...
import { getMaturity, selectWines, styleFamily, type Maturity } from '@/utils/cellar';
...
export default function CellarCollection({ wines, mode = 'cellar', locale = 'en', loading = false, error, onRetry, onAdd, onUpdate, onDelete, onConsume }: CellarCollectionProps) {
  ...
  return <main className={`collection-page ${journal ? 'journal-page' : ''}`}>
    ...
    <section id="collection" className="wine-collection">
      ...
      {loading ? <div className="wine-grid" aria-label={pt ? 'Carregando vinhos' : 'Loading wines'} aria-busy="true">{[1, 2, 3, 4, 5, 6].map(id => <div className="wine-skeleton" key={id}><div /><span /><span /></div>)}</div> : filtered.length ? <div className={view === 'grid' ? 'wine-grid' : 'wine-list'}>
