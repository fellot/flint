...
function tastingDate(value: string | null, locale: 'en' | 'pt') {
  const date = value ? new Date(value) : null;
  if (!date || !Number.isFinite(date.getTime())) return locale === 'pt' ? 'Data não registrada' : 'Date not recorded';
  const formatted = new Intl.DateTimeFormat(locale === 'pt' ? 'pt-BR' : 'en-CA', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC' }).format(date);
  return `${locale === 'pt' ? 'Consumido em' : 'Consumed'} ${formatted}`;
}
...
export default function CellarEssentials({ wines, locale = 'en', loading = false, error = false, onRetry }: Props) {
  ...
  const [scope, setScope] = useState<'all' | 'cellar' | 'journal'>('all');
  const matches = useMemo(() => new Map(CELLAR_ESSENTIALS.map(entry => [entry.id, getEssentialMatches(entry.id, wines)])), [wines]);
  const journalMatches = useMemo(() => new Map(CELLAR_ESSENTIALS.map(entry => [entry.id, getEssentialJournalMatches(entry.id, wines)])), [wines]);
  const represented = Array.from(matches.values()).filter(items => items.length > 0).length;
  const tasted = Array.from(journalMatches.values()).filter(items => items.length > 0).length;
  const inventoryAvailable = !loading && !error;
  const query = normalize(search.trim());
  const filtered = CELLAR_ESSENTIALS.filter(entry =>
    (tier === 'all' || entry.tier === tier) &&
    (scope === 'all' || !inventoryAvailable || !!(scope === 'cellar' ? matches : journalMatches).get(entry.id)?.length) &&
  ...
  const hasFilters = category !== 'all' || tier !== 'all' || search !== '' || scope !== 'all';
  const resetFilters = () => { setCategory('all'); setTier('all'); setSearch(''); setScope('all'); };
  ...
  return <main className="flint-subpage essentials-page">
    ...
    <section className="essentials-controls" aria-label={pt ? 'Explorar o guia' : 'Browse the guide'}>
      ...
      <div className="essentials-filter-summary">
        ...
        <div>
          <label className="essentials-match-filter">{pt ? 'Mostrar' : 'Show'}<select aria-label={pt ? 'Correspondências nos seus vinhos' : 'Matches in your wines'} value={scope} disabled={!inventoryAvailable} onChange={event => setScope(event.target.value as typeof scope)}><option value="all">{pt ? 'Todos os estilos' : 'All styles'}</option><option value="cellar">{pt ? 'Na minha adega' : 'In my cellar'}</option><option value="journal">{pt ? 'No meu diário' : 'In my journal'}</option></select></label>
      ...
      {CATEGORIES.map((group, index) => {
        ...
        return <section key={group.id} className={`essentials-chapter essential-${group.id}`} aria-labelledby={`chapter-${group.id}`}>
          ...
          {entries.map(entry => {
            ...
            const tastings = journalMatches.get(entry.id) || [];
            return <details className="essential-entry" key={entry.id}>
              <summary>
                ...
                <div className="essential-entry-end"><div className="essential-match-list">{inventoryAvailable && bottles.length > 0 && <span className="essential-match" title={pt ? 'Na sua adega' : 'In your cellar'}><Check size={12} />{pt ? 'Na adega' : 'In cellar'}</span>}{inventoryAvailable && tastings.length > 0 && <span className="essential-match essential-journal-match" title={pt ? 'No seu diário' : 'In your journal'}><BookOpen size={12} />{pt ? 'No diário' : 'In journal'}</span>}</div><ChevronDown className="essential-expand" size={19} /></div>
              </summary>
              <div className="essential-detail">
                ...
                <div className="essential-your-cellar">
                  <section className="essential-matched-section" aria-label={pt ? 'Vinhos na sua adega' : 'Wines in your cellar'}>
                    <p className="eyebrow"><WineIcon size={13} />{pt ? 'NA SUA ADEGA' : 'IN YOUR CELLAR'}</p>
                    {!inventoryAvailable ? <p>{loading ? (pt ? 'Consultando suas garrafas…' : 'Checking your bottles…') : (pt ? 'Seus vinhos estão indisponíveis no momento.' : 'Your wines are unavailable right now.')}</p> : bottles.length ? <ul>{bottles.map(wine => <li key={wine.id}><strong>{wine.bottle}</strong><span>{wine.vintage || (pt ? 'Sem safra' : 'Non-vintage')} · {wine.quantity} {pt ? (wine.quantity === 1 ? 'garrafa' : 'garrafas') : (wine.quantity === 1 ? 'bottle' : 'bottles')}{wine.location ? ` · ${wine.location}` : ''}</span></li>)}</ul> : <p>{pt ? 'Nenhuma correspondência clara no estoque.' : 'No clear match in your current stock.'}</p>}
                  </section>
                  <section className="essential-matched-section essential-journal-section" aria-label={pt ? 'Vinhos no seu diário' : 'Wines in your journal'}>
                    <p className="eyebrow"><BookOpen size={13} />{pt ? 'NO SEU DIÁRIO' : 'IN YOUR JOURNAL'}</p>
                    {!inventoryAvailable ? <p>{loading ? (pt ? 'Consultando suas degustações…' : 'Checking your tastings…') : (pt ? 'Seu diário está indisponível no momento.' : 'Your journal is unavailable right now.')}</p> : tastings.length ? <ul>{tastings.map(wine => {
                      const score = typeof wine.myRating === 'number' && Number.isFinite(wine.myRating) && wine.myRating >= 0 && wine.myRating <= 100 ? wine.myRating : null;
                      return <li key={wine.id}>
                        <strong>{wine.bottle}</strong>
                        <span>{wine.vintage || (pt ? 'Sem safra' : 'Non-vintage')} · {tastingDate(wine.consumedDate, locale)}</span>
                        <div className="essential-tasting-score">{score !== null ? <><span>{pt ? 'Sua nota' : 'Your score'}</span><strong>{score}<small>/100</small></strong></> : <span>{pt ? 'Ainda sem nota' : 'Not rated yet'}</span>}{wine.fromCellar === false && <span className="essential-external-tasting">{pt ? 'Fora da adega' : 'Outside the cellar'}</span>}</div>
                        {wine.myComment?.trim() && <blockquote className="essential-tasting-comment"><span>{pt ? 'Sua anotação' : 'Your note'}</span>{wine.myComment}</blockquote>}
                      </li>;
                    })}</ul> : <p>{pt ? 'Nenhuma correspondência clara no seu diário.' : 'No clear match in your journal yet.'}</p>}
                  </section>
                  {inventoryAvailable && !bottles.length && !tastings.length && <small>{pt ? 'Se você já tem ou provou este estilo, confira a região, as uvas e o tipo no cadastro do vinho.' : 'Already own or have tried this style? Check the region, grapes and style saved on the bottle.'}</small>}
                </div>
              </div>
            </details>;
          })}
