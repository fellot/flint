...
export default function CellarTable({ wines, locale, journal, sort, onSort, onView, onEdit, onDrink, onDelete, onReview }: Props) {
  const pt = locale === 'pt';
  const columns: { key: WineSortKey; label: string }[] = [
    ...
    ...(!journal ? [{ key: 'rating' as const, label: pt ? 'Nota' : 'Rating' }] : []),
    ...(journal ? [
    ...
    <div className="compact-table-scroll" role="region" aria-label={pt ? 'Tabela de vinhos' : 'Wine table'} tabIndex={0}>
      <table className="compact-wine-table">
        ...
        <tbody>{wines.map(wine => <tr key={wine.id}>
          ...
          {!journal && <td className="table-number">{wine.rating ?? '—'}</td>}
