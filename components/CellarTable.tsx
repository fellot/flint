...
import { ArrowDown, ArrowUp, ArrowUpDown, BookOpen, Edit3, GlassWater, Trash2 } from 'lucide-react';
...
export default function CellarTable({ wines, locale, journal, sort, onSort, onView, onEdit, onDrink, onDelete }: Props) {
  const pt = locale === 'pt';
  const columns: { key: WineSortKey; label: string }[] = [
    ...
    { key: 'rating', label: pt ? 'Nota' : 'Rating' },
    { key: 'location', label: pt ? 'Localização' : 'Location' },
  ];
  const statuses = { consumed: pt ? 'Consumido' : 'Enjoyed', gifted: pt ? 'Presenteado' : 'Gifted', sold: pt ? 'Vendido' : 'Sold', in_cellar: pt ? 'Na adega' : 'In cellar' };
  return <>
